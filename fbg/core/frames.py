...
@dataclass(frozen=True)
class DebugResponse:
    """Ответ 30 03 — одиночная развёртка по всем каналам сразу.

    Раскладка ✅ подтверждена скринингом (KB_04, N14 закрыт)::

        30 03 | LEN(4) | [ Канал(2) | Усиление(2) | ADC(2) × 2551 ] × N_каналов

    Для нашего прибора это 6 + 4 × 5106 = 20430 байт, LEN = 0x00004FCE.

    ⚠️ **Кадр телеметрии в это тело не входит.** PDF описывает ответ как
    «частоты + ADC», и это верно лишь отчасти: частоты приходят отдельной
    датаграммой `30 02` непосредственно **перед** ответом `30 03`. То есть
    одна команда порождает два ответа с разными парами (ID, FC).

    `payload` сохраняется рядом с разобранными блоками: правило KB_05 №3
    требует, чтобы сырые байты были доступны до и независимо от разбора.
    """

    blocks: tuple[AdcBlock, ...]
    """Блоки АЦП по каналам, в порядке следования в кадре."""

    payload: bytes
    """Сырое тело ответа без заголовка и поля LEN."""

    @property
    def channels(self) -> int:
        """Сколько блоков каналов пришло в ответе."""
        return len(self.blocks)


@dataclass(frozen=True)
class UndocumentedResponse:
    """Ответ на недокументированную команду 10 02 (KB_04, N17 — **открыт**).

    Наблюдённый скринингом ответ::

        10 02 04 00  →  10 02 00 14 | 05 DC | 0A 80 | 00 00 × 6
                                      1500     2688

    **Смысл полей неизвестен.** Поэтому модель не даёт им имён и ничего
    не пересчитывает: тело отдаётся и сырыми байтами, и разложенным
    на 16-битные слова — второе только потому, что вся остальная
    структура протокола 16-битная, а не потому, что это доказано.

    Штатное ПО использует эту команду вместо `10 05` и `10 06` при старте
    и повторяет её в простое как keepalive. Мы её ни в опрос, ни в watchdog
    не включаем — назначение полей неизвестно (решение чата №5).
    """
    ...
    words: tuple[int, ...]
    """То же тело, разложенное на 16-битные big-endian слова."""
