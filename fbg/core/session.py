...
Не разбирает тело ответа `30 03`: раскладка неизвестна (вопрос N14), тело
отдаётся сырым.
...
#: Ответы, которые могут не поместиться в одну датаграмму (вопрос D5):
#: `30 07` — 5112 байт, `30 03` — около 21 КБ.
...
class Session:
    ...
    def _starts_long_response(self, pending: _Pending, data: bytes) -> bool:
        ...
        ⚠️ Гипотеза. Сколькими датаграммами приходят `30 07` (5112 байт)
        и `30 03` (около 21 КБ) — открытый вопрос D5, захвата нет. Здесь
        принято, что первая датаграмма несёт заголовок с полным LEN,
        а остальные — продолжение без собственного заголовка. Если прибор
        режет иначе, менять придётся этот метод, а не кодек.
        """
    ...
    def debug_once(self) -> Result[DebugResponse]:
        """30 03 — одна развёртка в отладочном режиме; тело отдаётся сырым.

        Раскладка тела неизвестна (вопрос N14), поэтому кодек проверяет
        только заголовок. Сессия собирает ответ целиком по объявленному LEN
        и отдаёт байты как есть — интерпретировать их нечем.
        """
        if self.state is not SessionState.IDLE:
            return _fail(
                SessionErrorKind.WRONG_STATE,
