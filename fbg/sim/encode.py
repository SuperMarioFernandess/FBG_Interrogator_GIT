...
✅ После скрининга 27.08.2026 раскладки кадра телеметрии (N4), ответа `30 07`
и ответа `30 03` (N14) подтверждены реальным захватом — см. KB_06, раздел 4.
Синтетика осталась только там, где реального захвата нет, и помечена явно.
"""
...
#: Сырой код «пик не найден» — ✅ факт, подтверждённый скринингом (N3 закрыт):
#: прибор пишет в свободную позицию 0x000000, сохраняя байт индекса.
#: Имя оставлено прежним, чтобы не переписывать сцены и тесты, но статус
#: изменился: раньше это был стимул теста, теперь наблюдённое поведение.
MISSING_STIMULUS = 0x000000

#: Максимум трёхбайтового поля частоты.
FREQ_FIELD_MAX = 0xFFFFFF

#: Тело ответа на недокументированную команду 10 02, ✅ снято с прибора:
#: `05 DC | 0A 80 | 00 00 × 6`. Смысл полей неизвестен (N17), поэтому это
#: литерал из захвата, а не что-то вычисляемое.
SIM_UNDOCUMENTED_BODY = bytes.fromhex("05DC0A80" + "0000" * 6)
...
def encode_adc_block(channel: int, gain_mode: int, gain_level: int, adc: np.ndarray) -> bytes:
    """Блок одного канала: Канал(2) Усиление(2) ADC(2)×N.

    ✅ Один и тот же блок используется телом ответа `30 07` (один блок)
    и телом ответа `30 03` (по блоку на канал) — обе раскладки подтверждены
    скринингом.
    """
    return struct.pack(">H", channel) + bytes([gain_mode, gain_level]) + adc.astype(">u2").tobytes()
...
def encode_raw_adc(
    ...
    ✅ Скрининг: полный ответ 5112 байт, LEN = 0x000013F8.
    """
    return _mode_response(
        0x07, encode_adc_block(channel, gain_mode, gain_level, adc), len_width, len_field
    )


def encode_debug(payload: bytes, len_width: int, len_field: int | None = None) -> bytes:
    """30 03 — одиночная развёртка по всем каналам.

    ✅ Раскладка тела подтверждена скринингом (N14 закрыт):
    `[Канал(2) Усиление(2) ADC(2)×2551] × N_каналов`, 20430 байт для 4 каналов.
    Кадра телеметрии внутри **нет** — его симулятор шлёт отдельной датаграммой
    `30 02` перед этим ответом, как это делает прибор.

    Тело формирует вызывающий (`Scene.debug_payload`), этот модуль надевает
    заголовок.
    """
    return _mode_response(0x03, payload, len_width, len_field)


def encode_undocumented(payload: bytes) -> bytes:
    """10 02 — ответ на недокументированную команду (N17).

    Симулятор воспроизводит **ровно те байты, что пришли с прибора**, потому
    что смысл полей неизвестен и вычислять их не из чего. См. `SIM_UNDOCUMENTED_BODY`.
    """
    return _read_response(0x02, payload)
