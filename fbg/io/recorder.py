...
class Recorder:
    ...
    def _unique_path(self, now: datetime) -> Path:
        ...
        Номер дополняется нулями до трёх знаков (`_002`, а не `_2`) ради
        одного свойства: **сортировка имён по алфавиту обязана совпадать
        с порядком создания**. Без дополнения `_10` встаёт перед `_2`,
        и склейка файлов по `sorted(glob(...))` даёт перемешанные куски
        записи. Опираться вместо этого на время создания файла нельзя:
        на Windows его разрешение грубее, чем интервал между ротациями,
        и порядок получается произвольным — именно так это и вскрылось
        в CI чата №7. Дополнение работает до 999 совпадений в одну секунду;
        больше означало бы, что лимит ротации задан бессмысленно малым.

        Порядок восстанавливается и без имён — по полю `file_part` в шапке,
        и оно остаётся главным источником. Имя обязано ему не противоречить.
        """
        stem = f"{self._config.prefix}_{now.strftime('%Y%m%d_%H%M%S')}"
        path = self._config.directory / f"{stem}.csv"
        suffix = 2
        while path.exists():
            path = self._config.directory / f"{stem}_{suffix:03d}.csv"
            suffix += 1
        return path
