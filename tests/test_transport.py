...
def test_endpoint_слушает_любой_адрес_по_умолчанию() -> None:
    """🔴 Р29: `local_ip` обязан быть `0.0.0.0`, и это требование, а не удобство.

    Прибор отвечает **широковещательно** на `255.255.255.255:8001` — ✅ скрининг
    27.08.2026, все 4937 ответов. Bind на конкретный адрес интерфейса на Windows
    широковещательные датаграммы не получит: приложение замолчит при полностью
    исправной связи, и выглядеть это будет как отказ сети.

    Отдельный тест, а не строка в проверке умолчаний, потому что менять это
    значение нельзя даже «на время отладки», а причина по самому значению
    не видна.
    """
    assert Endpoint().local_ip == "0.0.0.0"
    assert Endpoint().bind_address == ("0.0.0.0", 8001)


def test_endpoint_не_фильтрует_по_source_порту_по_умолчанию() -> None:
    """N16 закрыт (прибор шлёт с 4567), но фильтр по порту остаётся выключенным.

    Решение чата №5. Фильтр по порту не отсекает ничего сверх обязательного
    фильтра по IP: датаграмма с IP прибора и другого порта всё равно от прибора.
    Зато при смене source-порта включённый фильтр даёт полное молчание
    приложения при исправной связи — тот же класс отказа, из-за которого
    `local_ip` обязан быть `0.0.0.0`. Цена ошибки асимметрична, поэтому
    умолчание — False.
    """
    assert Endpoint().strict_source_port is False
    assert Endpoint(strict_source_port=True).strict_source_port is True
...
def test_строгая_сверка_порта_выключена_по_умолчанию() -> None:
    """N16 закрыт (source-порт 4567), но умолчание остаётся False.

    Обоснование — в `test_endpoint_не_фильтрует_по_source_порту_по_умолчанию`
    и в комментарии к полю: фильтр по порту не добавляет ничего к фильтру
    по IP, а его ошибка стоит полного молчания приложения.
    """
    assert Endpoint().strict_source_port is False
