...
def test_линейная_кривая() -> None:
    """`c0 + c1·λ` считается точно."""
    sensor = temperature_sensor(c0=1.0, c1=2.0, c2=0.0)
    assert apply_curve(sensor, 3.0) == pytest.approx(7.0)


def test_квадратичная_кривая() -> None:
    """Третий член `c2·λ²` участвует наравне с остальными."""
    sensor = temperature_sensor(c0=1.0, c1=2.0, c2=3.0)
    assert apply_curve(sensor, 3.0) == pytest.approx(1.0 + 6.0 + 27.0)
...
def test_все_десять_типов_считаются_одинаково() -> None:
    ...
    for sensor_type in SensorType:
        sensor = temperature_sensor(id=f"S{int(sensor_type)}", type=sensor_type, c0=5.0, c1=0.0)
...
def test_датчик_переезжает_вслед_за_слотом() -> None:
    ...
    """
    sensor = temperature_sensor(id="T2", expected_nm=STAND_NM_2, c0=0.0, c1=1.0)
...
def test_выход_за_верхний_предел_помечается_но_значение_остаётся() -> None:
    ...
    """
    sensor = temperature_sensor(c0=0.0, c1=1.0, up_limit=1000.0)
    reading = evaluate(sensor, np.array([STAND_NM_1]))
    assert reading.status is ReadingStatus.OUT_OF_LIMITS
    assert reading.value == pytest.approx(STAND_NM_1)
    assert reading.ok


def test_выход_за_нижний_предел() -> None:
    """Симметрично верхнему."""
    sensor = temperature_sensor(c0=0.0, c1=1.0, down_limit=2000.0)
...
def test_внутри_пределов_статус_ok() -> None:
    ...
    sensor = temperature_sensor(c0=0.0, c1=1.0, down_limit=1500.0, up_limit=1600.0)
...
def test_знак_компенсации_задаётся_коэффициентом_а_не_кодом() -> None:
    ...
    без = evaluate(temperature_sensor(id="0", c1=0.0), row).value
    вверх = evaluate(
        temperature_sensor(id="A", c1=0.0, compensation=TempCompensation("T1", 1.5, 20.0)),
        row,
        reference_value=30.0,
    ).value
    вниз = evaluate(
        temperature_sensor(id="B", c1=0.0, compensation=TempCompensation("T1", -1.5, 20.0)),
        row,
        reference_value=30.0,
    ).value
...
def test_компенсация_на_опорном_значении_равном_базе_ничего_не_меняет() -> None:
    ...
    sensor = temperature_sensor(id="S1", c0=3.0, c1=0.0, compensation=comp)
...
def test_evaluate_all_считает_опорный_раньше_зависимого() -> None:
    ...
    reference = temperature_sensor(id="T1", expected_nm=STAND_NM_1, c0=0.0, c1=0.0, c2=0.0)
...
def test_evaluate_all_разводит_датчики_по_каналам() -> None:
    ...
    первый = temperature_sensor(id="A", channel=0, expected_nm=1545.0, c0=0.0, c1=1.0)
    второй = temperature_sensor(id="B", channel=1, expected_nm=1545.0, c0=0.0, c1=1.0)
    readings = evaluate_all([первый, второй], frame([1545.0, math.nan], [1545.2, math.nan]))
    assert readings["A"].value == pytest.approx(1545.0)
    assert readings["B"].value == pytest.approx(1545.2)
...
def test_потеря_опорного_датчика_гасит_зависимый_но_не_остальные() -> None:
    ...
    independent = temperature_sensor(id="S2", expected_nm=STAND_NM_2, c0=0.0, c1=1.0)
    ...
    assert readings["S2"].value == pytest.approx(STAND_NM_2)
...
def test_круговорот_json_сохраняет_датчик_целиком() -> None:
    """Всё, что влияет на расчёт, переживает запись и чтение."""
    sensor = temperature_sensor(
        id="S1",
        c2=1e-6,
        up_limit=100.0,
...
def test_отсутствие_имени_подставляет_идентификатор() -> None:
    ...
    assert restored.name == "S1" and restored.c1 == 0.0
